...
function bilgisayarinSecimi() {
  ...
  let bSecimi; 
  bSecimi = Math.floor(Math.random() * 3);
  if (bSecimi === 0) {
    return "Taş";
  } 
  else if (bSecimi === 1) {
...
function sesliHarfSayaci(metin) {
  ...
  const sesliHarfDizisi = ["a", "e", "ı", "i", "o", "ö", "u", "ü", "A", "E", "I", "İ", "O", "Ö", "U", "Ü"];
  let sayac = 0;
  for (let i = 0; i < metin.length; i++) {
    let karakter = metin[i];
    if (sesliHarfDizisi.includes(karakter)){
      sayac++
    }
  }
